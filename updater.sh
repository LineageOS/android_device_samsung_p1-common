...
if /tmp/busybox test -e /dev/block/bml7 ; then
    ...
    if $IS_GSM ; then
        ...
        if ! -e /system/vendor/modem.bin ; then
            cp /tmp/modem.bin /system/vendor/modem.bin
        fi
        mkdir -p /system/addon.d
        if ! -e /system/addon.d/20-modem.sh ; then
            cp /tmp/20-modem.sh /system/addon.d/20-modem.sh
            chmod +x /system/addon.d/20-modem.sh
        fi
