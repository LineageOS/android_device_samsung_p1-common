...
if /tmp/busybox test -e /dev/block/bml7 ; then
    ...
    if $IS_GSM ; then
        # if modem.bin doesn't exist on system partiton partition, format the partition and copy it
        if ! /tmp/busybox test -e /system/vendor/modem.bin ; then
            if ! /tmp/busybox mount -t ext4 /dev/block/mmcblk0p2 /system ; then
                /tmp/busybox echo "Cannot copy modem.bin to system partition."
                exit 5
            else
